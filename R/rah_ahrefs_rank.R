#' Export the URLs and their rankings.
#'
...
#'     ahrefs_rank \tab int    \tab - \tab + \tab URL Rating of the target.
#'     }
#'
...
#'               \code{RAhrefs::rah_ahrefs_rank(target = "ahrefs.com", token = "0123456789",
...
rah_ahrefs_rank <- function(target,
  ...
  data_df <- do.call(rbind.data.frame, as.list(data_list$pages))
  index <- sapply(data_df, is.factor)
  data_df[index] <- lapply(data_df[index], as.character)
  return(data_df)
}
# b <- rah_ahrefs_rank(target = "ahrefs.com", limit = 10)
# b <- rah_ahrefs_rank(
#   target = "ahrefs.com",
#   limit = 2,
