...
#'     refip     \tab string \tab + \tab + \tab The referring IP address that links to the target.                                \cr
#'     refdomain \tab string \tab + \tab + \tab The domain name for the referring IP address.                                     \cr
#'     backlinks \tab int    \tab + \tab + \tab The number of backlinks from the referring IP address with particular domain name.
#'     }
#'
...
#'               \code{RAhrefs::rah_refips(target = "ahrefs.com", token = "0123456789",
...
rah_refips <- function(target,
  ...
  data_df <- do.call(rbind.data.frame, data_list$refips)
...
# b <- rah_refips(target = "ahrefs.com", limit = 10)
# str(b)
