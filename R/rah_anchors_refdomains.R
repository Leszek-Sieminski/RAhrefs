#' Export connection between anchors and domains. Can be used to get all referring domains with specified anchor.
#'
...
#'     anchors   \tab int    \tab - \tab + \tab Number of different anchor texts that are used in backlinks to the target on the referring domain.\cr
#'     backlinks \tab int    \tab + \tab + \tab Number of external backlinks found that are using the anchor text.                                \cr
#'     refdomain \tab string \tab + \tab + \tab Referring domain that contains at least one backlink using the anchor text.
#'     }
#'
...
#'               \code{RAhrefs::rah_anchors_refdomains(target = "ahrefs.com", token = "0123456789",
...
#' cond_where <- RAhrefs::rah_condition_set(cond_1)
#'
#' # downloading
#' b <- RAhrefs::rah_anchors_refdomains(
#'   target   = "ahrefs.com",
#'   limit    = 2,
#'   where    = cond_where,
...
# b <- rah_anchors_refdomains(target = "ahrefs.com", limit = 10)
# str(b)
